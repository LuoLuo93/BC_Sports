 ...
 * 无关键字只返回前 30 名 + 全表真实统计；带关键字全库按姓名模糊搜索（可搜到
 * 30 名以外的人，名次为全榜绝对名次，命中过多截前 500 条）。
 * 领奖台切片与触底分页在前端完成。
 */
...
/**
 * 分页查询排名列表。
 * 无关键字:领奖台(全榜前三)占用榜单前三名,列表从第 4 名起(只到前30名);
 * 搜索时:领奖台保持全榜前三不变,列表显示全库全部命中(从第 1 名起,含前三名与30名以外)。
