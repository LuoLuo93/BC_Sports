<template>
  <div class="page-container">
        ...
        <el-card shadow="never" class="search-card">
          <el-form :model="query" inline>
            <el-form-item label="客户id">
              <el-input v-model="query.customerId" placeholder="请输入客户id" clearable @keyup.enter="handleSearch" />
            </el-form-item>
            <el-form-item label="首次添加人">
              <el-input v-model="query.firstAdder" placeholder="请输入首次添加人" clearable @keyup.enter="handleSearch" />
            </el-form-item>
            <el-form-item>
        ...
        <el-card shadow="never">
          <template #header>
            <div class="card-header-row">
              <span class="card-header-title">客户首次添加记录</span>
              <el-button v-if="hasPermission('bi:first-add:import')" type="warning" size="small" :icon="Upload" @click="showImportDialog = true">批量导入</el-button>
            </div>
          </template>
          ...
          <div class="table-responsive">
            <el-table v-loading="loading" :data="tableData" border stripe empty-text="暂无数据">
              ...
              <el-table-column prop="firstAddTime" label="首次添加时间" min-width="160" show-overflow-tooltip />
              <el-table-column prop="firstAdderDept" label="首次添加人部门" min-width="140" show-overflow-tooltip />
              <el-table-column prop="firstAdderStore" label="首次添加人店铺" min-width="140" show-overflow-tooltip />
              <el-table-column prop="firstAdder" label="首次添加人" min-width="120" show-overflow-tooltip />
            </el-table>
          </div>
    ...
    <!-- 批量导入弹窗 -->
    <el-dialog v-model="showImportDialog" title="批量导入客户首次添加记录" width="520px" destroy-on-close @open="resetImportState">
...
<script setup>
defineOptions({ name: 'FirstAddImport' })
...
async function loadData() {
  loading.value = true
  try {
    const res = await getFirstAddPage({ pageNum: query.pageNum, pageSize: query.pageSize, customerId: query.customerId, firstAdder: query.firstAdder })
...
async function handleDownloadTemplate() {
  ...
  try {
    const res = await getFirstAddTemplate()
    const blob = new Blob([res], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
    const url = window.URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = '客户首次添加导入模板.xlsx'
    a.click()
...
async function submitImport() {
  ...
  try {
    ...
    const res = await importFirstAdd(formData)
