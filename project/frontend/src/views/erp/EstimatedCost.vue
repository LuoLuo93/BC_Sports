<template>
  <div class="page-container">
        ...
        <el-card shadow="never" class="search-card">
          <el-form inline>
            <el-form-item label="物料编号">
              <el-input v-model="query.materialNumber" placeholder="请输入物料编号" clearable style="min-width:150px;max-width:200px" @keyup.enter="onSearch" />
            </el-form-item>
            ...
            <el-form-item label="物料名称">
              <el-input v-model="query.materialName" placeholder="请输入物料名称" clearable style="min-width:150px;max-width:200px" @keyup.enter="onSearch" />
            </el-form-item>
            <el-form-item>
              <el-button type="primary" :icon="Search" @click="onSearch">搜索</el-button>
              <el-button :icon="RefreshRight" @click="onReset">重置</el-button>
              <el-button v-if="hasPermission('erp:estimatedCost:import')" type="success" :icon="Upload" @click="showImportDialog = true">批量导入</el-button>
            </el-form-item>
          </el-form>
        </el-card>
        ...
        <el-card shadow="never">
          ...
          <div class="table-responsive">
            <el-table v-loading="loading" :data="tableData" border size="small" height="100%" :empty-text="hasSearched ? '暂无数据' : '请输入查询条件后点击搜索'">
              ...
              <el-table-column prop="MATERIAL_NUMBER" label="物料编号" width="180" show-overflow-tooltip fixed="left" />
    ...
    <!-- 导入弹窗 -->
    <el-dialog v-model="showImportDialog" title="批量导入预估成本" width="520px" @close="resetImportState">
      <el-upload
        drag
        :auto-upload="false"
        :limit="1"
      ...
      >
        ...
        <template #tip>
          <div class="el-upload__tip">
            仅支持 .xlsx / .xls 格式，文件不超过 100MB。
            <el-button link type="primary" :loading="templateLoading" @click="handleDownloadTemplate">下载导入模板</el-button>
          </div>
        </template>
      </el-upload>

      <el-alert v-if="importResult" :type="importResult.fail > 0 ? 'warning' : 'success'" :closable="false" style="margin-top:16px">
        <template #title>
          导入完成：共 {{ importResult.total }} 行，成功 {{ importResult.success }} 行，失败 {{ importResult.fail }} 行
        </template>
      </el-alert>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="importLoading" :disabled="!selectedFile" @click="handleImport">开始导入</el-button>
      </template>
    </el-dialog>
