          ...
          <div class="table-responsive">
            <el-table v-loading="onboardLoading" :data="onboardData" border stripe empty-text="暂无数据">
              ...
              <el-table-column label="入职日期" width="110" align="center">
                <template #default="{ row }">{{ row.enrollInDate || '-' }}</template>
              </el-table-column>
              ...
              <el-table-column label="操作" width="200" align="center" fixed="right">
                <template #default="{ row }">
                  <el-button v-if="row.syncStatus !== 1 && (row.staffStatus === 'IN_SERVICE' || row.staffStatus === 'PROBATION') && hasPermission('ihr:onboarding:sync')" type="primary" plain size="small" @click="handleOnboardSyncOne(row)">同步企微</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
          ...
          <div class="table-responsive">
            <el-table v-loading="adjLoading" :data="adjData" border stripe empty-text="暂无数据">
              ...
              <el-table-column label="变动日期" width="110" align="center">
                <template #default="{ row }">{{ row.modifyDate || '-' }}</template>
              </el-table-column>
              <el-table-column label="员工状态" width="100" align="center">
                <template #default="{ row }">
                  <el-tag :type="row.staffStatus === 'IN_SERVICE' ? 'success' : 'danger'" size="small">
              ...
              <el-table-column label="操作" width="200" align="center" fixed="right">
                <template #default="{ row }">
                  <el-button v-if="row.syncStatus !== 1 && (row.staffStatus === 'IN_SERVICE' || row.staffStatus === 'PROBATION') && hasPermission('ihr:update:sync')" type="primary" plain size="small" @click="handleAdjSyncOne(row)">同步企微</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
        ...
        <el-card shadow="never">
          ...
          <div class="table-responsive">
            <el-table v-loading="leaveLoading" :data="leaveData" border stripe empty-text="暂无数据">
              ...
              <el-table-column label="操作" width="200" align="center" fixed="right">
                <template #default="{ row }">
                  <el-button v-if="row.syncStatus !== 1 && hasPermission('ihr:leaving:sync')" type="primary" plain size="small" @click="handleLeaveSyncOne(row)">同步企微</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
...
<script setup>
defineOptions({ name: 'IhrEmployee' })
...
async function loadOnboardData() {
  ...
  try { const res = await getIhrOnboardingPage(onboardQuery); onboardData.value = res.data?.records || []; onboardTotal.value = res.data?.total || 0 } finally { onboardLoading.value = false }
}
...
async function loadAdjData() {
  ...
  try { const res = await getIhrUpdatePage(adjQuery); adjData.value = res.data?.records || []; adjTotal.value = res.data?.total || 0 } finally { adjLoading.value = false }
}
...
async function loadLeaveData() {
  ...
  try { const res = await getIhrLeavingPage(leaveQuery); leaveData.value = res.data?.records || []; leaveTotal.value = res.data?.total || 0 } finally { leaveLoading.value = false }
}
...
async function handleAdjSyncOne(row) {
  try { const res = await syncQywxUpdate(row.staffId); ElMessage.success(res.data || '同步成功') } catch { /* interceptor showed error */ } finally { loadAdjData() }
}
async function handleLeaveSyncOne(row) {
  try { const res = await syncQywxLeaving(row.employeeId); ElMessage.success(res.data || '同步成功') } catch { /* interceptor showed error */ } finally { loadLeaveData() }
}
