    ...
    <el-card shadow="never">
      <template #header>
        <div class="card-header-row">
          <span class="card-header-title">入职员工列表</span>
          <div>
            <el-button v-if="hasPermission('ihr:onboarding:sync')" type="success" size="small" :icon="Refresh" :loading="syncIhrLoading" @click="handleSyncIhr">从IHR同步</el-button>
            <el-button v-if="hasPermission('ihr:onboarding:sync')" type="warning" size="small" :icon="Refresh" :loading="syncQywxLoading" @click="handleSyncQywx">同步到企微</el-button>
          </div>
        </div>
      </template>
      ...
      <div class="table-responsive">
        <el-table v-loading="loading" :data="tableData" border stripe empty-text="暂无数据">
          ...
          <el-table-column label="操作" width="200" align="center" fixed="right">
            <template #default="{ row }">
              <el-button v-if="row.syncStatus !== 1 && row.staffStatus === 'IN_SERVICE' && hasPermission('ihr:onboarding:sync')" type="primary" plain size="small" @click="handleSyncOne(row)">同步企微</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
<script setup>
defineOptions({ name: 'IhrOnboarding' })
import { ref, onMounted } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import { getIhrOnboardingPage, syncIhrOnboarding, syncQywxOnboarding, syncQywxOnboardingByEmployee } from '@/api/ihr'
...
const { loading, tableData, total, query, loadData, handleSearch, resetQuery } = usePageQuery(getIhrOnboardingPage, { staffName: '', staffNo: '', syncStatus: undefined })
const syncIhrLoading = ref(false)
const syncQywxLoading = ref(false)

async function handleSyncIhr() {
  await ElMessageBox.confirm('确定从IHR系统同步入职数据？', '提示', { type: 'warning' })
  syncIhrLoading.value = true
  try { await syncIhrOnboarding(); ElMessage.success('同步任务已触发'); loadData() } finally { syncIhrLoading.value = false }
}
async function handleSyncQywx() {
  await ElMessageBox.confirm('确定将所有入职员工同步到企业微信？', '提示', { type: 'warning' })
  syncQywxLoading.value = true
  try { await syncQywxOnboarding(); ElMessage.success('同步任务已触发'); loadData() } finally { syncQywxLoading.value = false }
}
async function handleSyncOne(row) {
  await syncQywxOnboardingByEmployee(row.employeesId); ElMessage.success('同步任务已触发'); loadData()
}
