<template>
  <div class="page-container">
    <el-tabs v-model="activeTab">
      <el-tab-pane label="尺码组列表" name="data">
    ...
    <el-dialog v-model="showImportDialog" title="批量导入尺码组" width="520px" destroy-on-close @open="resetImportState">
      ...
      <el-tab-pane label="导入日志" name="log" lazy>
        <ImportLogPanel ref="logPanel" :fetcher="fetchSizeGroupLogPage" />
      </el-tab-pane>
    </el-tabs>
...
const fetchSizeGroupLogPage = (params) => request.get('/api/sticker/size-group/import-log/page', { params })
...
async function handleSave() {
  ...
  // 校验尺码明细：尺码编码/名称都必填 + 尺码编码组内唯一
  const seenCodes = new Set()
  for (let i = 0; i < (form.sizes || []).length; i++) {
    const s = form.sizes[i]
    const code = (s.sizeCode || '').trim()
    const name = (s.sizeName || '').trim()
    if (!code && !name) continue // 空行忽略
    if (!code) {
      ElMessage.warning(`第${i + 1}行：尺码编码不能为空`)
      return
    }
    if (!name) {
      ElMessage.warning(`第${i + 1}行：尺码名称不能为空`)
      return
    }
    const key = code.toLowerCase()
    if (seenCodes.has(key)) {
      ElMessage.warning(`第${i + 1}行：尺码编码「${code}」在本组内重复`)
      return
    }
    seenCodes.add(key)
  }
  const validSizes = (form.sizes || []).filter(s => s.sizeCode?.trim() && s.sizeName?.trim())
