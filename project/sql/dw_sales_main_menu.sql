...
-- ==========================================================
-- 2. 导入按钮权限 + 导入日志表(BC_SPORTS schema, identity 自增ID)
-- ==========================================================

-- 2.1 导入按钮权限 (menu_type=2, 不在侧边栏显示 visible=0)
INSERT INTO BC_SPORTS_SYS_MENU
  (ID, PARENT_ID, MENU_NAME, ICON, MENU_TYPE, PATH, PERMISSION, SORT, STATUS, VISIBLE,
   DESCRIPTION, ICON_COLOR, CREATE_TIME, UPDATE_TIME, CREATE_BY, UPDATE_BY, DELETED)
SELECT 'BI_DW_SALES_IMPORT', 'BI_DW_SALES', '导入', NULL, 2,
       NULL, 'bi:dw-sales:import', 2, 1, 0,
       '批量导入销售明细Excel', NULL, SYSTIMESTAMP, SYSTIMESTAMP, 'admin', 'admin', 0
  FROM DUAL
 WHERE NOT EXISTS (SELECT 1 FROM BC_SPORTS_SYS_MENU WHERE ID = 'BI_DW_SALES_IMPORT');

-- 2.2 授权给超管角色 (role_id='1')
INSERT INTO BC_SPORTS_SYS_ROLE_MENU (ID, ROLE_ID, MENU_ID, CREATE_TIME, CREATE_BY)
SELECT RAWTOHEX(SYS_GUID()), '1', 'BI_DW_SALES_IMPORT', SYSTIMESTAMP, 'admin'
  FROM DUAL
 WHERE NOT EXISTS (SELECT 1 FROM BC_SPORTS_SYS_ROLE_MENU WHERE ROLE_ID = '1' AND MENU_ID = 'BI_DW_SALES_IMPORT');

-- 2.3 导入日志表(结构同 BC_SPORTS_BUDGET_LOG)
DECLARE
  v_cnt NUMBER;
BEGIN
  SELECT COUNT(*) INTO v_cnt FROM user_tables WHERE table_name = 'BC_SPORTS_DW_SALES_IMPORT_LOG';
  IF v_cnt = 0 THEN
    EXECUTE IMMEDIATE q'[
      CREATE TABLE BC_SPORTS_DW_SALES_IMPORT_LOG (
    ...
    DBMS_OUTPUT.PUT_LINE('BC_SPORTS_DW_SALES_IMPORT_LOG created');
  ELSE
    DBMS_OUTPUT.PUT_LINE('BC_SPORTS_DW_SALES_IMPORT_LOG already exists, skip');
  END IF;
END;
/
