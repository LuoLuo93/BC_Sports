...
import com.bcsport.admin.ihrmapper.IhrEmployeeLeavingStatusMapper;
...
@Slf4j
@Service
public class IhrEmployeeLeavingServiceImpl implements IhrEmployeeLeavingService {

    private static final DateTimeFormatter DATE_FMT = DateTimeFormatter.ofPattern("yyyy-MM-dd");

    @Autowired
    private IhrEmployeeLeavingStatusMapper mapper;

    @Autowired
    private IhrEmployeeExclusionMapper exclusionMapper;

    @Override
    public PageResult<IhrEmployeeLeavingVO> pageLeavings(PageQuery pageQuery, IhrEmployeeLeavingQueryDTO queryDTO) {
        ...
        long total = mapper.countLeaving(staffName, staffNo, syncStatus, startDate, endDate, excludedStaffNos);

        int pageNum = pageQuery.getPageNum();
        int pageSize = pageQuery.getPageSize();
        long offset = (long) (pageNum - 1) * pageSize;

        List<IhrEmployeeLeavingVO> records = mapper.selectLeavingPage(
                staffName, staffNo, syncStatus, startDate, endDate, excludedStaffNos, offset, pageSize);

        PageResult<IhrEmployeeLeavingVO> result = new PageResult<>();
    ...
    public void markSyncSuccess(String employeeId, String staffName, String staffNo) {
        ...
        mapper.upsertByEmployeeId(status);
    }
